...
namespace Utility
{
    ...
    public static class Utils
    {
        ...
        public static string GetPostUTF8(HttpRequest req)
        {
            System.IO.Stream sm = req.InputStream;//获取post正文
            int len = (int)sm.Length;//post数据长度
            byte[] inputByts = new byte[len];//字节数据,用于存储post数据
            sm.Read(inputByts, 0, len);//将post数据写入byte数组中
            sm.Close();//关闭IO流
            string data = Encoding.UTF8.GetString(inputByts);
            return data;
        }
        ...
        {
            string now1 = System.DateTime.Now + System.DateTime.Now.Millisecond.ToString();
            now1 = now1.Replace(" ", "");
            now1 = now1.Replace(":", "");
            now1 = now1.Replace("-", "");
            return now1;
        ...
        public static string ChkSQL(string str)
        {
            ...
            {
                str2 = str.ToLower().Replace("'", "''").Replace("\\", "").Replace("&", "' || '&' || '").Replace(";", "\\;").Replace("=", "\\=");
            }
            return str2;
        }
        ...
        public static bool IsChinese(string strTmp)
        {
            bool isChinese = true;
